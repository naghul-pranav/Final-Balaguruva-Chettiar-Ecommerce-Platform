...
const MessagesPage = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        setLoading(true);
        const response = await axios.get('https://balaguruva-admin.onrender.com/api/contacts');
  ...
  const handleStatusChange = async (messageId, newStatus) => {
    try {
      await axios.put(`https://balaguruva-admin.onrender.com/api/contacts/${messageId}`, { status: newStatus });
